...
class WebhookProcessor:
    ...
    async def handle_return_declined(self, shop_domain: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            update_result = await db.returns.update_one(
                ...
                {
                    "$set": {
                        "status": "DENIED",  # Store uppercase for consistency
                        "decision": "denied", 
                        "decision_made_at": datetime.utcnow(),
                        "decision_made_by": "shopify",
                        "shopify_return_id": return_id,
    ...
    async def handle_return_cancelled(self, shop_domain: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Handle return cancellation from Shopify - Two-way sync"""
        try:
            return_id = str(payload.get("id"))
            order_id = str(payload.get("order_id", ""))
            ...
            print(f"🔄 Shopify webhook: Return {return_id} cancelled for {tenant_id}")
            ...
            # Check if status is already cancelled (prevent loops)
            if current_return.get("status", "").lower() == "cancelled":
                print(f"✅ Return {current_return['id']} already cancelled, skipping update")
                return {"action": "no_change", "return_id": current_return["id"]}
            ...
            audit_entry = {
                ...
                "description": f"Status updated to CANCELLED via Shopify webhook",
                "type": "shopify_sync"
            }
            ...
            if update_result.matched_count > 0:
                print(f"✅ Updated return {current_return['id']} status to cancelled from Shopify")
                return {"action": "return_cancelled", "return_id": current_return["id"], "shopify_return_id": return_id}
            else:
                print(f"❌ Failed to update return {current_return['id']}")
                return {"action": "update_failed", "return_id": current_return["id"]}
            
        except Exception as e:
            print(f"❌ Error handling return cancelled webhook: {e}")
            return {"action": "error", "error": str(e)}
