...
const Integrations = () => {
  ...
  const backendUrl = process.env.REACT_APP_BACKEND_URL || 'https://ecom-return-manager.preview.emergentagent.com';
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/integrations/shopify/status`, {
        ...
        headers: {
          ...
          'X-Tenant-Id': tenantId // Use authenticated user's tenant
        }
      });
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      // Redirect to OAuth install endpoint (include tenant in URL)
      const apiUrl = getApiUrl();
      const installUrl = `${apiUrl}/api/auth/shopify/install-redirect?shop=${encodeURIComponent(shopDomain)}&tenant_id=${encodeURIComponent(tenantId)}`;
